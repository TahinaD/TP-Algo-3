...
void print_skiplist(int value, void* environment) {
	fprintf((FILE*)environment, "%d ", value);
}
...
void test_construction(int num){
	...
	printf("Skiplist (%d) \n", skiplist_size(d));
	skiplist_map(d, print_skiplist, stdout);
	printf("\n");
	skiplist_delete(&d);
}
...
void test_search(int num){
	...
	if (input!=NULL) {
		unsigned int nb_values = read_uint(input), min_op = 1, max_op = 1, somme = 0, found = 0;
		for (unsigned int i=0; i < nb_values; i++) {
			int searched_value = read_int(input);
			unsigned int nb_operations = 1;

			if (skiplist_search(d, searched_value, &nb_operations)) {
				printf("%d -> true\n", searched_value);
				found++;
			}
			else
			...
			if (nb_operations < min_op)
				min_op = nb_operations;
			if (nb_operations > max_op)
				max_op = nb_operations;
			somme += nb_operations;
		}
...
void test_search_iterator(int num){
	...
	input = fopen(searchfromfile, "r");
	if (input!=NULL) {
		unsigned int nb_values = read_uint(input), min_op = 10, max_op = 1, somme = 0, found = 0;
		...
		for (unsigned int i=0; i < nb_values; i++) {
			...
			for (it = skiplist_iterator_begin(it) ; skiplist_iterator_value(it) != searched_value && !skiplist_iterator_end(it) ; it = skiplist_iterator_next(it))
				nb_operations++;
...
void test_remove(int num){
	...
	if (input!=NULL) {
		unsigned int nb_values = read_uint(input);
		for (unsigned int i=0; i < nb_values; i++) {
			int removed_value = read_int(input);
			skiplist_remove(d, removed_value);
		}
